	...
	public async packMany(
		...
		if (meId) {
			...
			for (const note of notes) {
				if (note.renote && (note.text == null && note.fileIds.length === 0)) { // pure renote
					...
					if (reactionsCount === 0) {
						...
						myReactionsMap.set(note.renote.id, pairs.length > 0 ? pairs.map(p => p.split('/')[1]) : null);
					} else {
					...
					if (note.id < oldId) {
						...
						if (reactionsCount === 0) {
							...
							const pairs = note.reactionAndUserPairCache.filter(p => p.startsWith(meId));
							myReactionsMap.set(note.id, pairs.length > 0 ? pairs.map(p => p.split('/')[1]) : null);
						} else {
			...
			for (const id of idsNeedFetchMyReaction) {
				const reactions = 	myReactions.filter(reaction => reaction.noteId === id).map(r => r.reaction);
